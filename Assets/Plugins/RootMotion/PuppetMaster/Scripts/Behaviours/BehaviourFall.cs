...
namespace RootMotion.Dynamics
{
	...
	public class BehaviourFall : BehaviourBase
	{
		...
		protected override void OnFixedUpdate(float deltaTime)
		{
			...
			float blend = Mathf.MoveTowards(puppetMaster.targetAnimator.GetFloat(blendParameter), blendTarget,
			...
			if (!endTriggered && canEnd && timer >= minTime && !puppetMaster.isBlending &&
			    puppetMaster.muscles[0].rigidbody.velocity.magnitude < maxEndVelocity)
			{
				endTriggered = true;
		...
		private float GetBlendTarget(float groundHeight)
		{
			...
			Vector3 verticalVelocity = V3Tools.ExtractVertical(puppetMaster.muscles[0].rigidbody.velocity,
				puppetMaster.targetRoot.up, 1f);
		...
		private float GetGroundHeight()
		{
			...
			if (Physics.Raycast(puppetMaster.muscles[0].rigidbody.position, -puppetMaster.targetRoot.up, out hit, 100f,
				    raycastLayers))
			{
				return hit.distance;
			}
